...
const scheduleSMS = (task, timeStr) => {
    ...
    if (!match) {
        console.log(`Warning: Invalid time format for task: "${task}". Reminder not set.`);
        return;
    }
    ...
    if (timeUntilReminder > 0) {
        setTimeout(async () => {
            sendSMS(`Reminder: "${task}" at ${timeStr}`);
        }, timeUntilReminder);
        console.log(`Success: Reminder scheduled for "${task}" at ${timeStr}`);
    } else {
        console.log(`Warning: Task "${task}" is too close or already passed. No reminder set.`);
    }
};
...
const processTasks = async (taskInput) => {
    try {
        ...
        console.log("Gemini API Key:", geminiApiKey); // Debugging: Check API key
        ...
        if (!responseContent) {
            console.log("Warning: No valid content returned.");
            return [];
        }
...
const askTask = () => {
    rl.question("What's Your Next Plan? (Describe your schedule) ", async (taskInput) => {
        ...
        if (extractedTasks.length === 0) {
            console.log("Warning: No valid tasks found.");
            askTask();
            return;
        }
        ...
        extractedTasks.forEach((taskStr) => {
            ...
            if (match) {
                ...
                console.log(`Success: Task Added: "${task}" at ${timeStr}`);
